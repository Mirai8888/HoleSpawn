...
def main() -> None:
    ...
    db_arg = lambda p: p.add_argument("--db", type=Path, default=None, help="DB path (default: outputs/holespawn.sqlite).")

    init_p = sub.add_parser("init", help="Create DB and tables.")
    db_arg(init_p)
    init_p.set_defaults(cmd="init")

    store_p = sub.add_parser("store", help="Store a run directory (behavioral_matrix + binding_protocol) into DB.")
    store_p.add_argument("run_dir", type=Path, help="Run directory (e.g. outputs/20260201_120000_username).")
    db_arg(store_p)
    store_p.set_defaults(cmd="store")

    search_p = sub.add_parser("search", help="Search profiles by agenda (descriptive query). Returns ranked list.")
    search_p.add_argument("--agenda", "-a", type=str, required=True, help="Descriptive query (e.g. interested in X).")
    ...
    if args.cmd == "store":
        ...
        if run_id is None:
            sys.stderr.write("error: run_dir must contain behavioral_matrix.json and metadata.json\n")
            sys.exit(1)
        print(run_id)
        return
